...
class Log {
    ...
    private const FORMAT_HTML = 1;
    private const FORMAT_ARRAY = 2;
    private const FORMAT_TEXT = 3;
    ...
    private static function add(string $type_log, $log, $auxiliar = null, bool $trace = false, string $tag = null): ?array {
        try {
            ...
            $log_info = [
                'type' => $type_log,
                'main' => self::format($log, self::FORMAT_ARRAY),
                'aux' => ($auxiliar != null ? self::format($auxiliar, self::FORMAT_ARRAY) : null),
                'date' => $date_time,
                'trace' => $str_trace,
                'tag' => $tag,
                'url' => URL,
            ...
            if ($format == 'html' || $format == 'all') {
                $texto = self::format($log, self::FORMAT_HTML);
                $texto .= ($auxiliar != null) ? '<br><p>' . self::format($auxiliar, self::FORMAT_HTML) : '';
     ...
     * @param int $format Formato do retorno
     *
     * @return string|array
     */
    private static function format($log, int $format = self::FORMAT_TEXT) {
        $array_log = [];
        ...
        if (is_a($log, 'Throwable')) {
            ...
            if ($format == self::FORMAT_TEXT) {
                $log = "Throwable: " . get_class($log) . "\n" .
                    "Mensagem: " . $log->getMessage() . "\n" .
                    "Code: " . $log->getCode() . " em " . $log->getFile() . "(" . $log->getLine() . ")\n" .
                ...
                $log = "Throwable: <i>" . get_class($log) . "</i><br>\n" .
                    "Mensagem: <i>" . str_replace("\n", "<br>\n", $log->getMessage()) . "</i><br>\n" .
                    "Code: <i>" . $log->getCode() . " em " . $log->getFile() . "(" . $log->getLine() . ")</i><br>\n" .
                    "Exception Trace:<br>\n" .
                    "<span style=\"color: #FD0017;\">" . str_replace("\n", "<br>\n", $log->getTraceAsString()) . "</span>";
            }
